                ...
                @if($jobOpenings->count())
                  <div class="table-container">
                    <table class="table mb-0" id="editableTable">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        @if($showRow)
                          <livewire:internal.job-opening-row id="nuevo" :key="time()" :objeto="$newJob" :nuevoObjeto='true' />
                        @endif
                        @foreach($jobOpenings as $jobOpening)
                          <livewire:internal.job-opening-row id="{{$jobOpening->id}}" :key="$jobOpening->id" :objeto="$jobOpening"/>
                        @endforeach
                      </tbody>
                    </table>
                  </div>
