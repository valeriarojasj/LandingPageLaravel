<div>
    ...
    @foreach ($jobs as $job)
        <div id="{{$job->id}}" class=" granDivModal fixed top-0 left-0 flex items-center justify-center w-full h-full opacity-0 pointer-events-none {{$job->id}} modal">
            <!--Modal Overlay-->
            <div id="modal-overlay{{$job->id}}" class="absolute w-full h-full bg-gray-900 opacity-50 modal-overlay {{$job->id}}"></div>
            <!--Cierre Modal Overlay-->
            <!--Contenedor del modal-->
            <div id="modal-container{{$job->id}}" class="z-50 hidden w-11/12 mx-auto overflow-y-auto bg-white rounded shadow-lg modal-container {{$job->id}} md:max-w-md">
                <div id="modal-close{{$job->id}}" class="absolute top-0 right-0 z-50 flex flex-col items-center mt-4 mr-4 text-sm text-white cursor-pointer modal-close">
                ...
                <div class="px-6 py-4 text-left modal-content ">
                    <!--Title-->
                        <div class="flex items-center justify-between pb-3">
                            ...
                            <div class="z-50 cursor-pointer modal-close {{$job->id}}">
