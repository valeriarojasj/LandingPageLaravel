<div>
    ...
    @foreach ($jobs as $job)
        <div class=" fixed top-0 left-0 flex items-center justify-center w-full h-full opacity-0 pointer-events-none modal {{$job->id}}">
            <!--Modal Overlay-->
            <div class="absolute w-full h-full bg-gray-900 opacity-50 modal-overlay {{$job->id}}"></div>
            ...
            <div class="z-50 hidden w-11/12 mx-auto overflow-y-auto bg-white rounded shadow-lg modal-container {{$job->id}} md:max-w-md">
                <div class="absolute top-0 right-0 z-50 flex flex-col items-center mt-4 mr-4 text-sm text-white cursor-pointer modal-close {{$job->id}}">
                ...
                <div class="px-6 py-4 text-left modal-content ">
                    ...
                    <!--Cierre del Title-->
                    <livewire:formulario :uuid="$job->id"/>
                </div>
