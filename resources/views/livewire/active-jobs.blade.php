<div>
    ...
    <div class='d-flex justify-between content-center'>
            ...
            @foreach ($jobs as $job)
                <div class="px-4 py-3 rounded-xl d-flex flex-column tarjetaBusqueda">
                    ...
                    <div class="mt-auto text-right">
                        <button data-toggle="modal" data-target="{{$job->id}}" class="px-4 py-1 font-bold text-white border border-gray-500 rounded-full btn btnModal modal-open hover:border-indigo-500 hover:text-indigo-500" >
                            Postularme
                        </button>
                    </div>
                </div>
    ...
    @foreach($jobs as $job)
        <div id="{{$job->id}}" class="fixed top-0 left-0 flex items-center justify-center w-full h-full opacity-0 pointer-events-none modal">
            <!--Modal Overlay-->
