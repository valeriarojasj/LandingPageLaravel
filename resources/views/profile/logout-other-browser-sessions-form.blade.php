<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Cerrar mis sesiones activas en otros navegadores y dispositivos.') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Puedes cerrar tus sesiones activas en otros navegadores y dispositivos. Si crees que alguien más tiene acceso a tu cuenta, actualiza también tu contraseña.') }}
        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Por favor ingresa tu contraseña pata confirmar que deseas cerrar todas las sesiones abiertas  de tu cuenta en otros navegadores y dispositivos.') }}

                <div class="mt-4" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" class="block w-3/4 mt-1" placeholder="{{ __('Contraseña') }}"
                                x-ref="password"
